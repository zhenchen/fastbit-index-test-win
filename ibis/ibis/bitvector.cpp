...
void ibis::bitvector::compress_compax() {
	...
	xrun() : isFill(false), fillBit(0), isDirty(0), dirtyPos(0), nWords(0), it(0) {};
	void decode() {
	    fillBit = (*it > HEADER1);
	    isFill = (*it > ALLONES);
...
*/	
	...
	last.decode();
	beforeLast.decode();
	std::cout<<"BfL:"<<beforeLast.isFill<<" "<<beforeLast.fillBit<<" ";
	std::cout<<"Last"<<last.isFill<<" "<<last.fillBit<<" ";
	std::cout<<"Current:"<<current.isFill<<" "<<current.fillBit<<" ";
	std::cout<<"ExistBeforeLast?"<<existBeforeLast<<std::endl;
...
void ibis::bitvector::compax2wah()
{
...
//	std::cout<<"cpxLength"<<cpxLength<<std::endl;
	...
	for(current.it;current.it<m_vec.end();current.it++)
	{
		std::cout<<std::hex<<*(m_vec.begin())<<" ";
	}
	std::cout<<std::endl;
	for(currentTmp.it;currentTmp.it<tmp_vec.m_vec.end();currentTmp.it++)
	{
		std::cout<<std::hex<<*currentTmp.it<<" ";
	}
	m_vec.swap(tmp_vec.m_vec);
}
